		...
		<?php if($status!='excel'){ ?>
			<div class="portlet-title">
				...
				<div class="actions">
					...
					<?php echo $this->Html->link( '<i class="fa fa-repeat"></i> Invoice Wise Report', '/SalesInvoices/InvoiceReport/'.@$url_excel.'&status=excel',['class' =>'btn btn-sm blue tooltips pull-right','target'=>'_blank','escape'=>false,'data-original-title'=>'Invoice Wise Report']); ?>
		...
		<?php } ?>
			<div class="portlet-body table-responsive">
				...
				<table class="table table-bordered table-hover table-condensed" width="100%" border="1">
					<thead>
						<tr>
							<th scope="col" colspan="24" style="text-align:left";>Sales Register According To  <?php if($from){ ?>Date From <?=$from ?><?php } ?><?php if($to){ ?>Date To <?=$to ?> <?php } ?><?php if($party_ids){ ?> Party <?php } ?><?php  if($invoice_no){ ?> Invoice No :<?=$invoice_no ?><?php } ?> </th>
						</tr>
						<tr>
